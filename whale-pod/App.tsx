...
import PodDetailsScreen from './src/screens/PodDetailsScreen';
...
function AppContent() {
  ...
  const [viewingPodId, setViewingPodId] = useState<string | null>(null);
  ...
  const navigation = {
    navigate: (screen: string, params?: any) => {
      ...
      } else if (screen === 'PodDetails' && params?.podId) {
        setViewingPodId(params.podId);
      } else if (screen === 'ApplicationReview' && params?.applicationId) {
        setReviewingApplicationId(params.applicationId);
      }
    },
    goBack: () => {
      setViewingUserId(null);
      setShowConnections(false);
      setViewingPodId(null);
...
// Show Application Review screen
if (reviewingApplicationId) {
  return (
    <ApplicationReviewScreen
      applicationId={reviewingApplicationId}
      onBack={() => setReviewingApplicationId(null)}
      navigation={navigation}
    />
  );
}

// Show Pod Details screen
if (viewingPodId) {
  return (
    <PodDetailsScreen
      pursuitId={viewingPodId}
      onBack={() => setViewingPodId(null)}
  ...
  const openPodDetails = (podId: string) => {
    setViewingPodId(podId);
  };
  ...
  />
      ...
      {currentScreen === 'Pods' && (
        <PodsScreen onOpenPodDetails={openPodDetails} />
      )}
